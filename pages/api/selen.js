...
const { normalize } = require('../../utils/triggerUtils.js'); // Importación estática
...
async function loadConfig() {
  ...
  try {
    ...
    logger.error("selen", "Error al cargar la configuración:", error.message, error.stack);
    throw error;
  }
}
...
async function optimizedFindTriggerContents(trigger) {
  ...
  if (contenidos && contenidos.length > 0) {
    ...
    logger.info("selen", "Contenidos guardados en caché");
  }
