...
async function optimizedFindTriggerContents(trigger) {
  ...
  try {
    ...
    logger.debug("selen", `Consultando caché para clave: ${cacheKey}`, { found: cached !== undefined });
    if (cached) {
    ...
    logger.debug("selen", `Trigger normalizado: ${normalizedTrigger}`);
    const contenido = triggerMap.get(normalizedTrigger) || "";
    const contents = contenido.length > 0 ? [contenido] : [];

    if (contents.length > 0) {
      const success = await cacheService.set(cacheKey, contents);
      logger.debug("selen", `Guardando en caché para clave: ${cacheKey}`, { success });
      logger.info("selen", "Contenido encontrado en CSV");
    } else {
      logger.warn("selen", `Trigger no encontrado en CSV: ${normalizedTrigger}`);
    }
    return contents;
  } catch (error) {
    logger.error("selen", `Error en optimizedFindTriggerContents para clave ${cacheKey}:`, error.message);
    return [];
  }
}
...
async function ejecutarTrigger(triggerRaw) {
  try {
    ...
    logger.debug("selen", `Trigger normalizado: ${trigger}`);
    ...
    if (!triggerData) {
      logger.warn("selen", `Trigger no encontrado, usando respuesta por defecto: ${trigger}`);
      return {
    ...
    // Guardar en cache
    const cacheSuccess = await cacheService.set(cacheKey, result);
    logger.debug("selen", `Guardando en caché para clave: ${cacheKey}`, { success: cacheSuccess });

    return result;
  } catch (error) {
    logger.error("selen", `Error al ejecutar trigger ${triggerRaw}:`, error.message, error.stack);
    return {
